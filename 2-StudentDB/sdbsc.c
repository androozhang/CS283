...
int del_student(int fd, int id)
{
    ...
    if (get_student_status == SRCH_NOT_FOUND) {
        ...
        return ERR_DB_OP;
    } else if (get_student_status == ERR_DB_FILE) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    } else if (get_student_status == NO_ERROR) {
        off_t myoffset;
        myoffset = id * STUDENT_RECORD_SIZE;
...
int count_db_records(int fd)
{
    ...
    student_t student_to_read = {0};
    int id = 0;
    off_t myoffset = id * STUDENT_RECORD_SIZE;
    off_t lseek_result = lseek(fd, myoffset, SEEK_SET);
    ssize_t read_result = read(fd, &student_to_read, STUDENT_RECORD_SIZE);
    ...
    while (read_result != 0) {
         if (memcmp(&student_to_read, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
            id++;
            myoffset = id * STUDENT_RECORD_SIZE;
            lseek_result = lseek(fd, myoffset, SEEK_SET);
            read_result = read(fd, &student_to_read, STUDENT_RECORD_SIZE);
        } else {
            counter++;
            id++;
            myoffset = id * STUDENT_RECORD_SIZE;
...
int print_db(int fd)
{
    ...
    while (read_result != 0) {
         if (memcmp(&student_to_read, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
            ...
            gpa = student_to_read.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, student_to_read.id, student_to_read.fname, student_to_read.lname, gpa);
            id++;
            myoffset = id * STUDENT_RECORD_SIZE;
            lseek_result = lseek(fd, myoffset, SEEK_SET);
            read_result = read(fd, &student_to_read, STUDENT_RECORD_SIZE);
